...
- **G3 review follow-ups (after Task 10, commit `fix(memory): only the API host creates the Rag.NET schema; enforce memory limits; console memory config`):**
  ...
  6. **Mutual exclusion.** `MemoryConfig`, `RalphRecallConfiguration` and `ILearningsMemory` are registered with `TryAddSingleton` in both extension methods, and `AddDaedalusMemory`'s doc states it is for hosts that do **not** call `AddDaedalusAgents`. **Superseded by the G3 follow-up (`97ff2d5`): a double call now throws.** The original claim that it was harmless was wrong — `UseRagNetMemory` is last-call-wins, so a later `AddDaedalusMemory` would flip `EnsureSchemaOnStartup` back to `false` on the API host, leaving nobody to create `rag_chunks` and every memory `index_pending`, silently. `ThrowIfMemoryAlreadyRegistered` refuses instead, and `LearningMemoryMapping.Truncate` in the same commit steps back a char rather than splitting a surrogate pair (a lone surrogate passes the aggregate's char-count validation but is not encodable as UTF-8, and Npgsql throws on write).
...
- **G6–G7 review follow-ups (after Task 18, commit `fix(memory): fail the browser suite loudly, scope API reads, clear recall on switch`):** (1) **The browser suite could not fail (blocker).** Narrowing the connection-string bug in Task 18 removed *a* cause; the amplifier was `E2EServerFixture`'s catch-all, which turned every startup exception into `BrowserSkipReason` → `Assert.Inconclusive` → “Skipped” → exit 0. Now only the container start is guarded, by `IsDockerUnavailable` (Docker.DotNet types, or an `ArgumentException` naming Docker — deliberately **not** a bare `SocketException`/`HttpRequestException`, since an unreachable Postgres carries one of those too); every later step propagates after recording `E2EServerFixture.StartupFailure`, and `BrowserTestBase` now `Assert.Fail`s on that and reserves `Inconclusive` for a genuinely absent environment. The fixture also probes `GET /api/agent-memories` beside `/health`, so degraded memory wiring fails the fixture. **Proved both directions:** a bogus `ConnectionStrings:daedalus` → `Failed: 5` with a `RagNetMemorySchemaInitializer` stack trace (previously: 4 silent skips); an injected `Docker.DotNet.DockerApiException` at container start → skipped, as intended. (`DOCKER_HOST` overrides are ignored by Testcontainers on this machine, hence the injection.) (2) **The controller now applies `MemoryScope.Includes` (Task 15 did not).** `MemoryQuery.AgentId = null` means “no agent filter”, so List/Get returned records pinned to *other* agents — the HTTP surface was more permissive than Thalos' own `memory__list`. Reads mirror the tool exactly: no `AgentId` on the query, visibility post-filtered with `scope.Includes`. Consequently **`agentId` is the caller's agent context, not a filter** (it widens the scope with that agent's pinned memories; the Agent page passes the session's agent id to both List and Get) — a deliberate divergence from Task 15's snippet, which set `MemoryQuery.AgentId`. Like the tool, the store pages *before* the filter, so a page may hold fewer than `PageSize` items and `Total` can over-count; documented on the controller. **Forget stays ownership-based** (as `memory__forget`): an owner may forget their own memory even when it is pinned to another agent. (3) **`GET /{id}` answers 404 for archived records** (M4 decided): `IMemoryStore.GetAsync` returns them, but `MemoryService.HydrateAsync` drops archived before recall, so hydration never needs them and a forgotten memory stays forgotten. (4) **Recall state is per session** (I3): `OnParametersSetAsync` clears `_recalled`/`_recalledIds`/`_recallStatus` next to the existing clears and reloads the panel (the visible set depends on the session's agent); after a forget empties a page, the panel falls back to the previous one (M8). (5) E2E seeds two more rows — a disposable one the forget scenario archives instead of a shared fixture row (M5), and one pinned to an unrelated agent that must stay invisible — and `StubAgentRuntime` filters its recall through `MemoryScope.Includes`, so the browser test covers the visibility rule too; the scenario reloads and re-opens the panel to prove the forget persisted (M7) and drops the vacuous `ErrorAlert` assertion (M6, memory errors raise a Radzen toast). (6) A remark on `ApiJsonSerializerContext` records that it is not in MVC's resolver chain today (M9). Memories controller facts 12 → 18; integration 338 → 343; browser suite 99, Agent category 5, `Skipped: 0`.
...
- **Task 19 (2026-08-17):** as written, plus one finding that changes what the plan's ArchUnit rule is worth.
  1. **Rag.NET's root namespace segment is `Rag`** (full namespaces are `Rag.NET`, `Rag.NET.Abstractions`, `Rag.NET.Models`, `Rag.NET.PgVector`, …; the package ships no XML docs, so this was read off the assemblies). The plan's `^Rag(\.|$)` is therefore correct as written.
  2. **ArchUnitNET does not synthesise types for assemblies it has not loaded**, so the plan's rule as specified would have passed *vacuously*: nothing in the solution references Rag.NET directly (only `Thalos.Memory.RagNet` does), and the loader saw no `Rag.*` type at all. `Rag.NET.Abstractions` and `Rag.NET.VectorStores.PgVector` are now loaded into the architecture beside the Thalos assemblies (both are already in the test output as transitive package deps). Proved: the load-bearing fact below failed with "There are no objects matching the criteria" *before* the assemblies were loaded, and passes after. The same reasoning is why adding `Thalos.NET.Memory` / `.Memory.RagNet` to `ThalosAssemblies` (the review note that prompted this task) actually matters: without them the `^Thalos(\.|$)` rules never saw a `Thalos.Memory.*` type either.
  3. `OnlyAgentsProject_DependsOn_RagNet` is **one fact looping over the five layers** (Domain, Application, Infrastructure, API, Web) rather than five near-identical facts; the `.Because` names the layer, so a failure still identifies it. Verified it bites by temporarily adding `Thalos.NET.Memory.RagNet` as a sixth "layer" — the rule failed with exactly that namespace, and the row was removed again. Its known positive is `MemoryRagNetAdapter_DoesDependOn_RagNet_RuleIsLoadBearing` (the Thalos adapter, not `Daedalus.Agents`, because Daedalus never names a Rag.NET type). ArchUnit facts 17 → 19; `Daedalus.Tests.Unit` 113 → 115.
  4. **README deviations from the plan's section list.** `Thalos:Memory:ExposeTools` *does* exist (it is a Thalos `MemoryOptions` member bound from the same section — only the per-agent `AgentMemorySettings` lacks it, §0.7 Task 6), so it is documented as a key. The SSE list is **five** kinds, not the four the plan names (`memory-quarantined` per §0.2b), and the kinds moved into their own table with payloads. Beyond the plan's list: an Ollama step and the `WaitForCompletion(migrations)` fix in "What Happens on Startup" and an operational bullet for the latter (both fixed on this branch); an authorization bullet for the memory scope/`developer` gate; the `agentId`-is-context caveat and the page-before-filter caveat from the G6–G7 review; `Thalos.NET.Testing` 0.1.0 → 0.2.0 and an ArchUnitNET row in the testing-libraries table; a Tech-stack row for the memory packages and one for the Ollama embeddings; `Npgsql 10.0.3` in the ORM row (the plan only asked for `Pgvector.EntityFrameworkCore` to go).
  5. **The plan's `rag_chunks` recovery recipe is wrong and was corrected.** Task 19 says "`rag_chunks` dimension mismatch → `DROP TABLE rag_chunks;` then restart, reindex rebuilds". It does not: `ReindexPendingMemoriesHostedService` sweeps with `ReindexOptions { PendingOnly = true }`, and rows indexed before the drop have `IndexPending = false`, so they would silently never be re-embedded and would stay invisible to recall. The README documents `DROP TABLE rag_chunks;` **plus** `UPDATE "AgentMemories" SET "IndexPending" = true WHERE NOT "IsArchived";` and says why the second statement is not optional; the diagram note carries the same caveat. (A `PendingOnly = false` mode exists on `ReindexOptions` — exposing it as an admin endpoint or a one-shot flag is a candidate follow-up, deliberately not taken here.)
  6. `docs/architecture-diagrams.md` §14: beyond the plan's ask (recall step + `memory-*` events) the `memory__remember` write path, the panel's `GET /api/agent-memories/{id}` hydration and three notes were added, and the **strangler graph was rewritten** — it still showed `StructuredLearnings`, `ILearningsRepository` and `IEmbeddingService`, all deleted in Task 11.
- **Task 21, documentation half (2026-08-17):** **step 1 does not apply and was not performed** — Thalos.NET 0.2.0 was already on nuget.org when execution started, so Task 1 pinned it directly and there was never a `packages-local/`, a `thalos-local` source or a `0.2.0-local.<stamp>` pin to undo (see the Task 1 bullet). `ROADMAP.md`/`MILESTONE.md` mark 1.2 `complete (2026-08-17; Thalos.NET 0.2.0 on nuget.org, #228)` with all three plan links, and `STATE.md` was rewritten as a 1.2 → 1.3 handoff: what shipped, no blocking decisions, the deferred M4/M5/M8 items and the two AppHost environment findings (stale local Postgres volume with a collation-version mismatch; the `WaitFor(migrations)` → `WaitForCompletion(migrations)` fix in `4d96d66`), and phase 1.3 (Skills, #229) as the next step. **Step 3 (`complete 1.2`, closing #228) and Task 20 (regression run, pre-push review, merge) stay with the coordinator**, so the roadmap rows are written ahead of those closing steps.
