...
- **Amendments after Task 24 review (2026-08-16):** AI.Sentinel 2.0.1's security detectors (incl. SEC-01 prompt injection) are all *semantic* — without `SentinelOptions.EmbeddingGenerator` they return Clean; **hosts must wire a real `IEmbeddingGenerator` (Daedalus: Ollama) or only the 14 lexical/operational detectors run**. `SentinelChatClientDecorator.Order = -1000` (innermost: scans exactly what reaches the provider). `AgentError.Quarantined` detail is `"<Severity>: <DetectorId>"` only (Reason goes to a Warning log — it can echo credential fragments); Sentinel rate-limit rejections (`PipelineResult == null`) map to `ProviderError`; the mapper unwraps Sentinel's `InvalidOperationException("Inner client … failed.", inner)` so provider errors keep their message; `UseAISentinel` twice is a no-op. Follow-ups: per-agent Sentinel identity via named pipelines; per-session rate limiting via `ChatOptions.AdditionalProperties["sentinel.session_id"]`.
- **Amendments after Tasks 22–23 review (2026-08-16):** `AnthropicChatClientProvider` holds one shared `AnthropicClient` per provider (`IDisposable`; the returned `IChatClient` is caller-owned but does not tear down the transport) and has an internal ctor with an injectable env accessor; `Microsoft.Extensions.Options.ConfigurationExtensions` 10.0.11 added for `OptionsBuilder.Bind`; `McpServerDefinition.ShutdownTimeout` (stdio; default 5 s — the MCP SDK waits the full timeout on dispose); `McpToolSource` validates its name at construction, disposes the client if `ListToolsAsync` fails, guards disposal with a `_disposed` flag + gate, maps `sse`/`http` to explicit `HttpTransportMode`s; the shared name rule lives in `Thalos.ToolSourceName.IsValid` (Abstractions) and is used by `ToolCatalog`, `LocalToolSource`, `McpToolSource`, `AddMcpServer`; MCP SDK 2.2.0 sanitises server-side tool exceptions to `isError:true` results (message hidden unless `McpException`); `.mcp.json` paths are CWD-relative and `${VAR}` expansion is not implemented; the `legacy` config test asserts `EffectiveType`.
- **Amendments after Tasks 20–21 review (2026-08-16):** post-persist notifications (`SessionCreated`, `SessionClosed`, `TurnCompleted`, `TurnFailed`) never fail the operation (`PublishPostPersistAsync`, log 206); `ThalosAgentRuntime` ctor logger defaults to null (works without `AddLogging`); `[Singleton(As = typeof(...))]` is used on the four generated registrations (plain `[Singleton]` double-registers / trips ZAI007); `ThalosOptions.Agents/ToolPolicies` are `IList<T>`; `OptionsAgentCatalog` snapshots at construction and throws a clear duplicate-agent-id error; the `IAgentRuntime` factory fails fast for a missing store *and* provider; `ThalosBuilder` uses `TryAddEnumerable` for type-based multi-registrations, has `AddToolSource(Func<IServiceProvider, IToolSource>)`, an `internal` ctor, and `AddLocalTools` validates `[ThalosToolType]` eagerly; `LocalToolSource.ScopedTool` forwards the per-call scope in `AIFunctionArguments.Services` and disposes disposable tool instances.
