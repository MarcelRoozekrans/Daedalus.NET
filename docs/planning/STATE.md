...
Merge #237 → tag + publish 0.1.0 images (decision 1); do (2) → publish Thalos 0.1.0 + switch pins → close #227 → `complete` phase 1.1 formally. Then start phase 1.2 (Memory: `Thalos.NET.Memory` port + Rag.NET adapter) with `superpowers:brainstorming` (design doc §11).
