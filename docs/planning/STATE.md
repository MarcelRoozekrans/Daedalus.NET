...
2. **Publish Thalos.NET 0.1.0 to nuget.org**: in the Thalos repo, tag `v0.1.0` and push the tag (release.yml needs the `NUGET_API_KEY` secret and the `nuget` environment approval); then in Daedalus set the six `Thalos.NET*` pins to `0.1.0`, delete `packages-local/` + the `thalos-local` source in `nuget.config`, commit `build: consume Thalos.NET 0.1.0 from nuget.org`, close #227.
