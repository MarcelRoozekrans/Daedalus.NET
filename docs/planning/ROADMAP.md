...
| 2.4 | The process as skills: the manufacturing steps written as skills rather than code, including an explicit review stage, so the process can change without a redeploy | Mixed | **active (2026-09-23)** — design `docs/plans/2026-09-23-phase-2.4-process-as-skills-design.md`, plan `docs/plans/2026-09-23-phase-2.4-process-as-skills-plan.md`: run manifest pinning skill versions and charter revisions, squad roles as charters over a config tool envelope, `POST /api/workflow-runs` + `manufacture__start`, and the `AGENT.md` loop as a `retrospect` node applied only on a human resume. Part A is Thalos.NET 0.10.0 | 2.3 |
