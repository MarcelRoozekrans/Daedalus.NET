...
> **Renumbered 2026-09-24.** A new phase 2.5 (run write authority) was inserted after 2.4, so Ralph retirement is now **2.6** and observability, the scout HTTP path and the manufacturing console moved to 2.7-2.9. Earlier prose here and in `STATE.md` that says "phase 2.5 deletes Ralph" was written before the insert and means 2.6.
...
| 2.4 | The process as skills: the manufacturing steps written as skills rather than code, including an explicit review stage, so the process can change without a redeploy | Mixed | **complete (2026-09-24)** — PR #276. Part A shipped **Thalos.NET 0.10.0**: skill version history, versioned role charters, agent revisions, and a run manifest pinning each node's agent revision and skill hash. Part B: `SkillVersions` with a backfill migration; squad roles as charters in `roles/` over a config tool envelope; `POST`/`GET /api/workflow-runs` and `manufacture__start`, closing 2.3's missing `work_intent` producer; process v5 with a proposing-only `retrospect` node; standing instructions applied to `AGENT.md` only on a human resume at the gate, authored only by retrospect; and an optional interval resync. The final review found a Critical — any node could plant the proposal the gate applies — fixed before merge. Suites: Unit 397, Unit.Application 425, Unit.Domain 356, Unit.Infrastructure 142, Integration 581. **The live proof stopped at `implement`:** all four nodes pinned through the real endpoint, and the implementer honestly reported `blocked` after 737,634 input tokens. The budget decorator then failed the node after the turn, so it does not cap spend. **Review, retrospect and the gate are unreachable by any live run** because a run cannot write — `roslyn__apply_*` is `developer`-only, and v5 routes `blocked` to a failed end. That is phase 2.5. Rulings, live-proof record and carried-forward list: `docs/plans/2026-09-23-phase-2.4-rulings.md` | 2.3 |
| 2.5 | Run write authority, standing instructions in the target workspace, and prompt caching: make the manufacture process reachable end to end. Three owner decisions of 2026-09-24. First, a run writes under a scoped grant tied to the developer who started it: only the `implement` node gets `roslyn__apply_*`, never `git__*` or `repoaction__*`, each write is logged with the starter's id, and publish stays behind the gate. Second, `AGENT.md` resolves against the run's target workspace rather than the host content root. Third, prompt caching goes into `Thalos.NET.Anthropic`, deciding whether cached reads count toward `MaxTotalTokens`. Ends with the live proof rerun that 2.4 could not complete: implement, review lenses, `checked[]`, retrospect, the gate, a resume, and `Succeeded`. Inserted 2026-09-24 | Backend | planned | 2.4 |
| 2.6 | Ralph retirement: delete the loop once 2.1–2.5 demonstrably do its job better | Refactor | planned | 2.5 |
| 2.7 | Observability: `ZeroAlloc.Telemetry` source-generated spans and metrics. Five OpenTelemetry packages are pinned today but `src` contains **zero** `ActivitySource` or `Meter`, so nothing emits a custom span. A manufacturing run that cannot be traced cannot be debugged | Backend | planned | 2.2 |
| 2.8 | The scout HTTP path: `ZeroAlloc.Rest` replaces the 6 hand-rolled `HttpClient` API clients, and `ZeroAlloc.Cache` fronts the **rate-limited** GitHub API the scout hits on every digest. One subsystem, so they land together rather than separately | Backend | planned | 2.1 |
| 2.9 | Manufacturing console: `ZeroAlloc.Flux` for Blazor state, covering the approval-gate and run-inspection surfaces 2.2 and 2.3 produce | UI | planned | 2.2, 2.3 |
