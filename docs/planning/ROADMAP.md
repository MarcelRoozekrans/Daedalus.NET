...
| 1.8 | Thalos.NET **0.6.0** release, docs, architecture-diagrams rewrite | Docs | pending | all | #233 | **Scoped 2026-09-21: the 1.0 tag is deliberately held back.** A 1.0 signals API stability, and Milestone 2 will reshape the framework — phase 2.2's durable workflow engine and 2.3's squad roster will very likely want changes to `ISubagentRunner` and the agent contracts. Tagging 1.0 now would force a choice between a fast 2.0 or holding the framework design back to preserve compatibility, and neither is worth the version number. So 1.8 delivers the documentation and the architecture-diagram rewrite, which are valuable immediately and are **not** invalidated by Milestone 2, and ships as 0.6.0. **1.0 is tagged once Milestone 2 has settled the agent contracts**, at which point it means something |
