...
| 1.4 | Channels: Telegram (+ CLI) via `IChannelAdapter` + `ZeroAlloc.Outbox` | active (started 2026-08-20) | 1.1 | #230 | design: `docs/plans/2026-08-20-thalos-channels-design.md` · plan A (Thalos.NET repo): `docs/plans/2026-08-20-thalos-channels-plan-a.md` — **merged 2026-08-21 via [Thalos.NET#41](https://github.com/MarcelRoozekrans/Thalos.NET/pull/41)** (937 tests incl. Testcontainers; breaking change: `IChannelAdapter.DeliverAsync` re-keyed `SessionId` → `ConversationId`). **0.4.0 not yet released** — release-please is `workflow_dispatch`-gated and has not been triggered. · plan B (Daedalus): not started, blocked until Thalos.NET 0.4.0 resolves from nuget.org |
